...
> [!IMPORTANT]
> Для работы требуется активированный **Сценарий обработки Allure отчетов**
...
Этот сценарий необходим, если есть потребность в сборе метрик из автотестов, привязки их к соответствующим КЕ с тем, чтобы позже открывать Сигналы в случае, например, превышения временных лимитов на выполнение автотестов.
...
<img src="./img/08.png" width="440">

Сценарий при этом будет с указанной частотой проверять 
А. наличие открытых Сигналов проваленных Автотестов
Б. если (А) найден - искать открытые Сигналы Порогов
В. если (Б) найдены - закрывать их.