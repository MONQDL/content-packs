...
Главным принципом для привязки любых объектов в Monq является корреляция некоторого общего признака этих объектов. В данном демонстрационном сценарии для корреляции Порогов и КЕ сравниваются метки `pod` метрик, помещенных в аннотацию `bindci` и кастомные атрибуты КЕ `commonPropertyAttr` + `commonPropertyKey`.
